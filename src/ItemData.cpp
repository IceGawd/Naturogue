...
ItemData::ItemData(shared_ptr<SDL_Texture> t, int w, int h, int af, int at, int sw, int sh, string n, double am, double sa, int m, int r, int st, bool ts, bool s, float tv, float f, float k, vector<WEAPON_PROPERTIES> pr, vector<Vector2f> p) {
	constructorBase(t, w, h, af, at, sw, sh, n, am, sa, m, r, st, ts, s, tv, f, k, pr);
	for (Vector2f v2f : p) {
...
void ItemData::constructorBase(shared_ptr<SDL_Texture> t, int w, int h, int af, int at, int sw, int sh, string n, double am, double sa, int m, int r, int st, bool ts, bool s, float tv, float f, float k, vector<WEAPON_PROPERTIES> pr) {
	texture = t;
	width = w;
	height = h;
	animationFrame = af;
